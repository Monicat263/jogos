...
def iniciar_rodadada():
    while (rodada <= max_tentativas):
        ...
        if (
                chute_convertido_utilizando_casting < 1 or chute_convertido_utilizando_casting >= 100):
            continue
        ...
        if acertou_numero_secreto:
            print(
                'Parabéns!!! Você acertou o número secreto é: {} e você fez {} pontos'.format(
                    numero_secreto, pontos))
            break
        else:
            if chute_maior_que_numero_secreto:
                print(
                    'Você errou!!! Dica: O número digitado é maior que o número secreto')
                if (rodada == max_tentativas):
                    print("O número secreto era {}. Você fez {}".format(
                        numero_secreto, pontos))
            elif chute_menor_que_numero_secreto:
                print(
                    'Você errou!!! Dica: O número digitado é menor que o número secreto')
                if (rodada == max_tentativas):
                    print("O número secreto era {}. Você fez {}".format(
                        numero_secreto, pontos))
        ...
        pontos_perdidos = abs(
            numero_secreto - chute_convertido_utilizando_casting)
